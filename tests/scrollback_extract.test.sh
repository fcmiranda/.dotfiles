...
grep -q "bind-key \"e\" run-shell \".*scrollback-extract.sh '#{pane_id}'" "$CONF" && ok "bind prefix+e+origin" || bad "bind prefix+e+origin"
grep -q "bind-key \"v\" run-shell \".*dir-peek.sh '#{pane_id}'" "$CONF" && ok "bind prefix+v+origin" || bad "bind prefix+v+origin"
